...
class MY_Migration extends CI_Migration {
	...
	public function __construct($config = array())
	{
		...
		if ( ! $this->db->table_exists('migrations'))
		{
			$this->dbforge->add_field(array(
				'module' => array('type' => 'VARCHAR', 'constraint' => 30),
	 ...
	 */
	public function version($module, $target_version)
	{
		$start = $current_version = $this->_get_version($module);
		$stop = $target_version;
		...
		foreach ($migrations AS $migration)
		{
			...
			$this->_update_version($module, $current_version);
		}
	...
	public function latest($module)
	{
		...
		return $this->version($module, (int) substr($last_migration, 0, 3));
	}
	...
	{
		return $this->version($module, $this->_migration_version);
	}
	 ...
	 * @return	bool
	 */
	protected function _update_version($module, $migrations)
	{
		$row = $this->db->get_where('migrations', array('module'=>$module))->row();
		if($row)
		{
			return $this->db->where('module', $module)->update('migrations', array(
				'version' => $migrations
			));
		}
		else
		{
			return $this->db->insert('migrations', array(
				'module' => $module, 'version' => $migrations
			));
		}
	}
	...
	public function execute_sql($file)
	{
		...
		foreach(explode("\n", $backup) as $line)
		{
			if(isset($line[0]) AND $line[0] != '#' AND strpos($line, '--') !== 0)
			{
				$sql_clean .= $line;
			}
		}

<script>
	$(function () {
			...
			toolbar: {
				fontfamily: {
					options: [
						...
						{
							label: ' 한컴돋움 (<span class="tx-txt">가나다라</span>)',
							title: '한컴돋움',
							data: '"한컴돋움",AppleGothic,sans-serif',
							klass: 'tx-gulim'
						},
						{
		...
		EditorJSLoader.ready(function (Editor) {
			...
			var existVal = $('#' + _target_form).find('input[name="<?php echo $input; ?>"]').val();
			if(existVal) {
			...
			// 체크박스 체크 저장되도록
			$("#tx_trex_container<?php echo $sequence; ?>").find('#tx_canvas_wysiwyg<?php echo $sequence; ?>').contents().on('click', 'input[type="checkbox"]', function () {

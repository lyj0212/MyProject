...
jQuery(function($){
	$('.sortablemenu').delegate('li:not(.placeholder)', 'dropped.st', function() {
		var $this = $(this), $pkey, $mkey, is_child;

		$pkey = $this.find('input._parent_key:first');
		is_child = !!$this.parent('ul').parent('li').length;

		if(is_child) {
			$pkey.val($this.parent('ul').parent('li').find('input._item_key:first').val());
		} else {
			$pkey.val('0');
		}
	});
...
jQuery(function($){
	...
	$('.sortablemenu .lined li').css('cursor', 'all-scroll');
